<template>
  <div class="home">
    ...
    <div v-for="item in list" :key="item" style="margin-top: 20px">
      <div>
        <span>涨跌幅：</span>
        <span :style="{color: item.f3 > 0 ? 'red' : 'green'}">{{ (parseInt(item.f3) / 100) + '%' }}</span>
    ...
    <a href="https://console.cloud.tencent.com/tke2/node-map/overview?rid=1&clusterId=cls-4qa3oxeg" target="_blank">referer腾讯云</a>
    <p>xx</p>
    <a href="https://intl.cloud.tencent.com/document/product/457/32189?from_cn_redirect=1" target="_blank" rel="noreferrer">noreferrer腾讯云</a>
  </div>
</template>
